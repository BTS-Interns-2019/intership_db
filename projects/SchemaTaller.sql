...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `tallermecanico`.`marcas_por_proveedor` (`id` INT, `proveedor` INT, `marca` INT);
...
USE `tallermecanico`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `tallermecanico`.`marcas_por_proveedor` AS select distinct `tallermecanico`.`proveedor`.`idProveedor` AS `id`,`tallermecanico`.`proveedor`.`name` AS `proveedor`,`tallermecanico`.`refaccion`.`marca_refaccion` AS `marca` from ((`tallermecanico`.`proveedor` join `tallermecanico`.`proveedor_refaccion` on((`tallermecanico`.`proveedor`.`idProveedor` = `tallermecanico`.`proveedor_refaccion`.`Proveedor_idProveedor`))) join `tallermecanico`.`refaccion` on((`tallermecanico`.`proveedor_refaccion`.`Refaccion_idRefaccion` = `tallermecanico`.`refaccion`.`idRefaccion`)));
